...
/**
 * Mutable bindle. Presently, one can only <em>add</em> information;
...
public class BindleBuilder implements Bindle {
   ...
   * ledger ID.
   * 
   * @param alias unique name across <em>this</em> bindle
   * @param path  skipledger (commitment) path
   * @param uri   optional. If not {@code null}, then unique across bindle
  ...
  public LedgerId declareLedger(LedgerInfo ledger, Path path) {
    ...
    if (findIdByAlias(ledger.alias()).isPresent())
      throw new IllegalArgumentException(
          "ledger alias '%s' is already defined in this bindle"
          .formatted(ledger.alias()));
    
    if (ledger.uri().isPresent() && findIdByUri(ledger.uri().get()).isPresent())
      throw new IllegalArgumentException(
          "ledger URI '%s' is already defined in this bindle"
   ...
   * 
   * @throws IllegalArgumentException
   *         if the bindle does not have sufficient information to validate
...
//                ref.toRn(), toId, ref.fromRn(), ref.fromColNo(), fromId));
  ...
  public ObjectBundle build() {
    return new ObjectBundle(this);
  }
