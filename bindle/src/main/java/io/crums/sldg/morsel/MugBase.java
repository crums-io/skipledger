...
/**
 * Base class for the {@linkplain Morsel}
 * implementations {@linkplain LazyMorsel}, {@linkplain ObjectMug},
 * and {@linkplain MorselFile}.
 * 
 * @see #ids()
 */
public abstract class MugBase implements Morsel {

  private final LedgerId[] ids;
  

  /**
   * Builder constructor. ({@linkplain MorselBuilder#ids()}
   * are pre-validated, sorted by {@linkplain LedgerId#id()}).
   */
  MugBase(MorselBuilder builder) {
    if (builder.isEmpty())
   ...
   */
  protected MugBase(Collection<LedgerId> ids) throws MalformedMorselException {
    this.ids = ids.toArray(new LedgerId[ids.size()]);
    Arrays.sort(this.ids, Mug.ID_COMP);
    verifyIds();
  }
  
  
  /** For subclass private constructors. Not validated */
  protected MugBase(LedgerId[] ids) {
    this.ids = ids;
  }
  
  /** For copy constructor (if any). */
  protected MugBase(MugBase copy) {
    this.ids = copy.ids;
  }
   ...
   *         
   */
  protected final void verifyIds() throws MalformedMorselException {
    if (ids.length == 0)
      throw new MalformedMorselException("empty morsel");
  ...
  protected final int indexOf(LedgerId id) throws IllegalArgumentException {
    int index = Arrays.binarySearch(ids, id, Mug.ID_COMP);
   ...
   * of the {@linkplain Mug} sub-interface.
   * 
   ...
   * {@linkplain MalformedMorselException}s on look-up failure.
   */
