...
/**
 * A well-formed, validated {@linkplain Morsel}.
 */
public class ObjectMug extends MugBase implements Mug {
  
  private final ObjectNug[] nugs;

  /**
   * Constructs an instance using a {@linkplain MorselBuilder}.
   * Since the argument type is guaranteed to be well-formed,
   * 
   * @param builder not empty
   */
  public ObjectMug(MorselBuilder builder) {
    super(builder);
  ...
  /**
   * Constructs an instance from the given {@linkplain MugBase}
   * implementation instance.
   * 
   * 
   * @param morsel a purported morsel
   ...
   */
  public ObjectMug(MugBase morsel)
      throws MalformedMorselException, HashConflictException {
    super(morsel);
    this.nugs = new ObjectNug[ids().size()];
    init(morsel);
  }
  ...
  /**
   * Constructs an instance from the given {@linkplain Morsel}.
   * 
   * @param morsel a purported morsel
   * @throws MalformedMorselException
   *         if the argument is malformed
   ...
   *         if a hash-proof in the argument fails
   */
  public ObjectMug(Morsel morsel)
      throws MalformedMorselException, HashConflictException {
  ...
  private void init(Morsel morsel) {
    var ids = ids();
    for (int index = 0; index < ids.size(); ++index) {
      Nugget nugget = morsel.getNugget(ids.get(index));
   ...
   * 
   * @return {@code new ObjectMug( LazyMug.load(in) )}
   */
  public static ObjectMug load(ByteBuffer in)
      throws
      SerialFormatException,
      MalformedMorselException, HashConflictException {
    
    var morsel = LazyMug.load(in);
    return new ObjectMug(morsel);
  }
